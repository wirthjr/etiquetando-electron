try {
  ...
  async function createWindow() {
    ...
    win = new BrowserWindow({
      width: 900,
      height: 600,
      icon: path.join(__dirname, 'imagem.png'),
      webPreferences: {
  ...
  app.whenReady().then(() => {
    ...
    try {
      ...
      console.log("Tray icon error: " + error)      
    }

    win.on('minimize', (event) => {
      event.preventDefault(); // Evita o comportamento padrão de minimizar
      win.hide(); // Esconde a janela
    });

    win.on('restore', () => {
      win.show(); // Mostra a janela quando restaurada
    });
  ...
  autoUpdater.on('error', (err) => {
    sendData('update', '😕 Erro: ' + err.message);
  });

  autoUpdater.on('update-downloaded', (info) => {
    sendData('update', '✅ Atualização baixada. Preparando para instalar...');
